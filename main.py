...
def main():
    ...
    with st.sidebar:
            ...
            if internal_id:
                ...
                if lat is not None and lon is not None:
                    ...
                    if pressure is not None:
                        auto_p = pressure
                        st.success(f"Infection pressure found: {auto_p:.4f}")
                        auto_ln_smp = auto_p
                    else:
        ...
        if sm_mode == "manual":
            ln_smp = st.number_input(T["pressure_val"], min_value=0.0, max_value=100.0, value=15.0)
        else:
    ...
    if st.button(T["calculate"]):
        ...
        with st.spinner("Running simulation..."):
            results = run_simulation(input_data, temp, p_weeks)
